...
- [x] As an SDET, I can automate registration & login with data-driven datasets.
- [x] As an SDET, I can automate account overview, open-account, transfer, and bill pay, asserting on balances.
- [x] As an SDET, I can automate find-transactions and negative cases.
- [x] As an SDET, I have ≥15 stable UI tests with resilient locators (no fixed sleeps). (24 UI tests)

## Sprint 4 — API & Data (Days 7-8)

- [x] As an SDET, I have a Postman collection covering the key REST services with assertions.
- [x] As an SDET, I have the same checks re-implemented as automated Playwright API tests.
- [x] As an SDET, I have one hybrid test (API seeds data, UI verifies it).
- [x] As an SDET, I have a seeded SQL database and a TypeScript module asserting query results.

## Sprint 5 — CI/CD, Reporting & Polish (Days 9-10)

- [x] As an SDET, I have a GitHub Actions workflow running the suite green on every push/PR to main.
- [x] As an SDET, I have the HTML report published as a build artifact.
- [x] As an SDET, I have a complete AI-usage log, README, and RTM.
- [x] As an SDET, I have a 1-2 page final report and a rehearsed demo.
